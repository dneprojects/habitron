...
async def async_setup_entry(
    ...
    # Snapshot before adding so a cover's deviating area is stamped only at
...
class HbtnShutter(HbtnAreaMixin, CoordinatorEntity[HbtnCoordinator], CoverEntity):
...
class HbtnBlind(HbtnShutter):
    ...
    async def async_set_cover_tilt_position(self, **kwargs: Any) -> None:
        """Set the tilt angle."""
        tmp_tilt_position = int(kwargs.get(ATTR_TILT_POSITION))  # type: ignore[arg-type]
        self._tilt_position = 100 - self._module.covers[self._nmbr].tilt
        sh_nmbr = self._nmbr + 1
        if self._module.mod_type[:16] == "Smart Controller":
            sh_nmbr -= 2  # map #3..5 to 1..3
